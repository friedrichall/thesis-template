...
Zur Laufzeit wird diese Spezifikation dann mittels einer State-Machine-basierten Umgebung interpretiert um ein Interaktionsverhalten abzubilden~\cite{VivianFrameworkGitLab2021}.
...
Sie besteht im Wesentlichen aus 4 Teilen: Interaktionselemente (Interaction Elements), Visualisierungselemente (Visualization Elements), Zustände (States) und Übergänge (Transitions)~\cite{VivianFrameworkGitLab2021}.
...
Multimodale Large Language Models (MLLMs) sind Modelle, welche im Gegensatz zu reinen LLMs nicht nur Text, sondern zudem Informationen wie Bild, Audio oder Video empfangen (Input), verarbeiten sowie ausgeben (Output) können.
Eine für diese Arbeit wichtige Fähigkeit ist ihr Verstehen von 2D-Bildern und darüber zu schlussfolgern.
...
Im Vergleich zu früheren, ähnlichen Arbeiten basieren MLLMs auf LLMs mit Milliarden Parametern sowie neuen Trainingsparadigmen, wie dem ``Multimodal Instruction Tuning''~\cite{liuVisualInstructionTuning}.
