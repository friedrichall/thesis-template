...
So wird vermieden, dass einzelne Agenten in der Spezifikationsgenerierung inkonsistente Entscheidungen treffen. Er besitzt ein tiefes Verständnis vom Vivian Framework und kann so einzelnen Teilobjekten bestimmte Rollen zuweisen, Zustände planen und entscheiden, welche Dateien der Spezifikation für die geplante Interaktion benötigt werden.
...
Die Rollen-Zuweisung ist dabei einer der Hauptaufgaben.
In diesem Schritt werden den Objekten einer 3D-Szene spezifische Rollen zugewiesen, wie \textit{Button}, \textit{Slider}  oder \textit{Screen}, welche das LLM direkt aus den einzelnen Elementen von SceneUnderstanding ableitet.
Beispielsweise wird einem \textit{FunctionButton} die Rolle eines \textit{Button} zugewiesen.
...
Das umfasst das Erstellen von Zustandsnamen, Beschreibungen und möglichen Auslösern (\textit{Triggern}), die diese Übergänge initiieren.
Die Zustände und Übergänge werden hier bereits geordnet beschrieben, allerdings erst im Functional-Specification-Generator-Schritt in Spezifikationsdateien umgewandelt.

\begin{lstlisting}[
	language=json,
	caption={Beispielausgabe des Interaction Planners: Elementrollen},
...
Die letzte Aufgabe des Interaction Planners umfasst die Bestimmung des Umfangs, also welche konkreten Spezifikationsdateien für die gegebene Szene erforderlich sind, da dies von deren Komplexität des Prototyps abhängt. Einfachere Prototypen mit vergleichsweise wenigen Elementen oder Funktionen benötigen gegebenenfalls nicht alle Dateien einer Spezifikation, während komplexere alle vier Dateien benötigen können.

Das vom Scene Analyzer ausgegebene \textit{SceneUnderstanding}-Objekt im JSON-Format sowie die natürlichsprachliche Interaktionsbeschreibung werden dazu vom Interaction Planner konsumiert und in einen strukturierten Interaktionsplan (\textit{InteractionPlan}) überführt. Dieser enthält ein vordefiniertes Format, welches Informationen über Element-Rollen, geplante Zustände und Übergänge sowie eine Liste der benötigten Spezifikationsdateien enthält. Darüber hinaus liefert das LLM noch eine zusammengefasste Begründung für die getroffenen Entscheidungen mit, welche es Nutzenden erleichtern soll Fehler zu finden und zu beheben. Ein Ausschnitt eines Interaktionsplans ist in Listing~\ref{interaction_planner_roles} zu sehen.
...
\begin{lstlisting}[
	language=json,
	caption={Beispielausgabe des Interaction Planners},
...
\section{Functional Specification Generator}
