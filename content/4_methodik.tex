...
Im ersten Schritt wird aus dem im Unity-Fenster (Abbildung~\ref{fig:unity_ui_empty}) ausgewählten 3D-Asset ein Prefab-Objekt erzeugt, welches später von Vivian zur Laufzeit instanziiert wird.
...
Um einem LLM Verständnis über ein 3D-Asset (in Unity GameObject genannt) zu geben wird zunächst eine strukturierte Szenenbeschreibung im JSON-Format mit dem Namen scene.json erzeugt.
Dazu wird die Hierarchie des Szenengraphen rekursiv traversiert und pro GameObject Informationen wie die ID, der Hierarchiepfad, Transformation, Materialien, Bounding Boxes und Mesh-Statistiken extrahiert.
...
Im Anschluss daran rendert Unity für das übergeordnete GameObject acht Ansichten, davon sechs orthografisch entlang der Hauptachsen sowie zwei isometrisch als 1024x1024-Pixel-PNGs.
%TODO LITERATUR warum 8?
...
Das ist besonders interessant für nachgelagerte MLLMs, da diese so den Zusammenhang zwischen Objekten in scene.json und denen in den Ansichten herstellen können.
