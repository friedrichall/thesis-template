...
Wie bereits in Kapitel X beschrieben, wird zur Interaktion mit dem VSG Unity als Benutzer*innenschnittstelle genutzt. Ein einem Fenster kann eine initiale Interaktionsbeschreibung eingegeben und das zu transformierende statische 3D-Asset ausgewählt werden. Durch die Verwendung vorhandener Geometrie adressiert dieser Schritt D2. Außerdem wird A6 adressiert, da hier natürlichsprachliche Beschreibungen entgegengenommen werden. Abbildung X veranschaulicht Unity als Schnittstelle in der Gesamtarchitektur zwischen Benutzer*in und KI-Pipeline.
...
Eine Skalierung ist durch dieses Vorgehen ebenfalls möglich: Durch Erweiterung bestehender oder Hinzufügen neuer Module könnten noch komplexere 3D-Objekte verarbeitet werden, als in dieser Arbeit betrachtet werden. Während ein einzelner LLM-Aufruf, wie in Kapitel~\ref{dawd} aufgezeigt, für diese Ausgabe nicht ausreichend wäre, ermöglicht die hier vorgestellte Pipeline durch die Zerlegung in einzelne Schritte sowie den Einsatz von Zwischenvalidierungen und iterativer Fehlerbehebung die Erzeugung korrekter Spezifikationen nach Vivian.
