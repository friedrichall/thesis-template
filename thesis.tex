\pdfminorversion=7
\documentclass[
    11pt,
    a4paper,
    sfdefaults=false,
    toc=chapterentrywithdots,
    twoside,openright,
    titlepage,
    parskip=half,
    headings=normal,  % reduces heading size
    listof=totoc,
    bibliography=totoc,
    index=totoc,
    captions=tablesignature,  % caption below table
    chapterprefix,
    listof=flat,
    numbers=noenddot,
    final
]{scrbook}


% details about your thesis
\newcommand{\titel}{KI-gestützte Transformation statischer in interaktive 3D-Objekte durch Benutzerinteraktion}
\newcommand{\artderarbeit}{Bachelorarbeit}  % {Bachelorarbeit,Masterarbeit}
\newcommand{\autor}{Friedrich Allenhof}
\newcommand{\studiengang}{Medieninformatik}  % {Informatik,Wirtschaftsinformatik,Medieninformatik}
\newcommand{\matrikelnr}{359\,4928}
\newcommand{\erstgutachter}{Prof.\,Dr.~Uwe Wienkop}
\newcommand{\zweitgutachter}{Louis Burk}
\newcommand{\betreuer}{M.Sc.\,~Martina Schmidt}
\newcommand{\logo}{figures/TH-Nuernberg-RGB.png}
\newcommand{\keywords}{Interaktive Prototypen, 3D-Objekte, KI-gestützte Transformation, Virtual Prototyping,
Generative KI, Large Language Models, Virtual Reality, Vivian Framework}
 

% custom head and foot
\usepackage[automark]{scrlayer-scrpage}
\pagestyle{scrheadings}
\ihead{\headmark}
\chead{}
\ohead{}
\ifoot*{}
\cfoot*{}
\ofoot*{\pagemark}
\renewcommand*\chaptermarkformat{\chapappifchapterprefix{\ }%
  \thechapter\enskip}

\RedeclareSectionCommand[tocindent=0pt]{section}
\RedeclareSectionCommand[tocindent=0pt]{subsection}
%\RedeclareSectionCommand[tocnumwidth=70pt]{chapter}

\usepackage{scrhack}

% other packages
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern,relsize,textcomp,csquotes}
\usepackage[htt]{hyphenat}
\usepackage{amsmath,amsfonts}
\usepackage[english,ngerman]{babel}  % flip for English thesis
\usepackage[final]{graphicx}
\usepackage{subcaption}
\usepackage{tikz}
\usetikzlibrary{arrows.meta, positioning, calc, shapes.geometric}
\usepackage[edges]{forest}
\usepackage{fontawesome5}
\usepackage{setspace,geometry,xcolor}
\usepackage{makeidx}
\usepackage{paralist,ifthen,todonotes}
\usepackage{url}
\usepackage[toc]{glossaries}
\usepackage{pdfpages}

\usepackage[backend=biber, style=ieee, url=true, doi=true]{biblatex}
\addbibresource{refs.bib}

% table setup
\usepackage{longtable}
\usepackage{array}
\usepackage{ragged2e}
\usepackage{lscape}
\usepackage{booktabs}
\usepackage{tabularx}
\usepackage{multirow}

% more breathing room in all tables
\renewcommand{\arraystretch}{1.3}
\setlength{\aboverulesep}{0.8ex}
\setlength{\belowrulesep}{0.8ex}

% PDF/A compliance
\begin{filecontents*}{\jobname .xmpdata}
\Title{\titel}
\Author{\autor}
\Subject{\titel}
\Keywords{\keywords} 
\Creator{\autor}
\end{filecontents*}

\usepackage[a-3b,mathxmp]{pdfx}
\hypersetup{
    bookmarksopen=true,
    bookmarksnumbered=true,
    bookmarksopenlevel=1,
    colorlinks=true,
    linkcolor=black,
    urlcolor=black,
    anchorcolor=black,
    citecolor=black,
    filecolor=black,
    menucolor=black,
    plainpages=false,
    hypertexnames=true,
    linktocpage=true
}

% configure your listings style
\usepackage{listings}
\usepackage{listingsutf8}

\lstset{
    basicstyle=\normalsize\ttfamily,
	tabsize=2,
	extendedchars=true,
	inputencoding=utf8,
	frame=single,
	showstringspaces=false,
	numbers=left,
    numbersep=5pt,
	numberstyle=\small,
	breakautoindent=true,
	breaklines=true,
	columns=fullflexible,
	captionpos=b,
    xleftmargin=1.5em,
    xrightmargin=1.5em,
    aboveskip=2em,
    belowskip=1em,
}
\lstdefinelanguage{json}{
    basicstyle=\normalsize\ttfamily,
    numbers=left,
    numberstyle=\scriptsize,
    stepnumber=1,
    numbersep=8pt,
    showstringspaces=false,
    breaklines=true,
    frame=single,
    xleftmargin=1.5em,
    xrightmargin=1.5em,
    aboveskip=2em,
    belowskip=1em,
}
\lstdefinestyle{prompt}{
    basicstyle=\normalsize\ttfamily,
	breaklines=true,
	breakatwhitespace=true,
	columns=fullflexible,
	frame=single,
	numbers=none,
	showstringspaces=false,
    xleftmargin=0em,
    xrightmargin=0em,
    aboveskip=2em,
    belowskip=1em,
    captionpos=b,
    float=htbp,
}
\lstdefinelanguage{agent-instructions}{
    basicstyle=\normalsize\ttfamily,
	breaklines=true,
	breakatwhitespace=true,
	columns=fullflexible,
	frame=single,
	numbers=none,
	showstringspaces=false,
	keepspaces=true,
	inputencoding=utf8,
	extendedchars=true,
    xleftmargin=0em,
    xrightmargin=0em,
    literate={≈}{{$\approx$}}1 {→}{{$\rightarrow$}}1 {—}{{---}}1 {–}{{--}}1 {„}{{\quotedblbase}}1 {“}{{\textquotedblleft}}1 {”}{{\textquotedblright}}1 {‚}{{\quotesinglbase}}1 {‘}{{\textquoteleft}}1 {’}{{\textquoteright}}1 {ä}{{\"a}}1 {ö}{{\"o}}1 {ü}{{\"u}}1 {Ä}{{\"A}}1 {Ö}{{\"O}}1 {Ü}{{\"U}}1 {ß}{{\ss}}1
}


% minted for syntax-highlighted code listings (requires -shell-escape and Pygments)
% Use newfloat with a separate file extension (.loml) so minted's listing float
% does not clash with the listings package's `.lol` (registered via scrhack).
% \usepackage[newfloat]{minted}
% \SetupFloatingEnvironment{listing}{fileext=loml}
% \usemintedstyle{friendly}

\usepackage{float}
\usepackage{needspace}

% page setup
% \setlength{\topskip}{\ht\strutbox}
\geometry{paper=a4paper,left=2.5cm,top=3.0cm,bindingoffset=.8cm}
\onehalfspacing
\frenchspacing
\clubpenalty = 10000
\widowpenalty = 10000 
\displaywidowpenalty = 10000

% some commands
\newcommand{\ua}{\mbox{u.\,a.\ }}
\newcommand{\zB}{\mbox{z.\,B.\ }}
\newcommand{\dahe}{\mbox{d.\,h.,\ }}
\newcommand{\bzw}{\mbox{bzw.\ }}
\newcommand{\bzgl}{\mbox{bzgl.\ }}
\newcommand{\eg}{\mbox{e.\,g.\ }}
\newcommand{\ie}{\mbox{i.\,e.\ }}
\newcommand{\wrt}{\mbox{w.\,r.\,t.\ }}
\newcommand{\etal}{\mbox{\emph{et\,al.\ }}}

% load glossary entries
% \makenoidxglossaries
\makeglossaries
\loadglsentries{glossary}

\begin{document}

\setcounter{secnumdepth}{3}  % numerate subsections
\setcounter{tocdepth}{2}  % ...but don't include them in toc

\frontmatter
\include{cover}

\include{content/0_abstract}\cleardoublepage

\tableofcontents
\cleardoublepage

\KOMAoptions{open=any}
\listoffigures
\clearpage

\listoftables
\clearpage

\renewcommand{\lstlistlistingname}{Listingverzeichnis}
\lstlistoflistings
\clearpage
\KOMAoptions{open=right}

\mainmatter
\include{content/1_einleitung}
\include{content/2_grundlagen}
\include{content/3_stand_der_forschung}
\include{content/4_ansätze}
\include{content/5_konzeption_implementierung}
\include{content/6_evaluation}
\include{content/7_ausblick}
\include{content/8_fazit}

\appendix
\cleardoublepage
\phantomsection
\addcontentsline{toc}{chapter}{Anhang}
\addtocontents{toc}{\protect\setcounter{tocdepth}{-1}}
\input{content/anhang_a}
\input{content/anhang_b}
\input{content/anhang_c}
\input{content/anhang_e}
\input{content/anhang_d}
\addtocontents{toc}{\protect\setcounter{tocdepth}{2}}

\backmatter
\printbibliography

% \printnoidxglossaries
\printglossary

\input{content/hinweis_nutzung_ki}

% download the following form (requires VPN) and complete it (hit save in your editor)
% https://www.th-nuernberg.de/fileadmin/global/Public_Docs/SB/SB_0009_FO_Pruefungsrechtliche_Erklaerung_und_Erklaerung_zur_Veroeffentlichung_der_Abschlussarbeit_public.pdf
\includepdf{SB_0009_FO_Pruefungsrechtliche_Erklaerung_und_Erklaerung_zur_Veroeffentlichung_der_Abschlussarbeit_public_print.pdf}\cleardoublepage

\end{document}
